...
if __name__ == '__main__':
    df = pandas.read_csv('F:/myStuff/供应预测/数据/常规数据/常规妥投.csv', engine='python', skip_blank_lines=True
                         , parse_dates=['real_delv_day'])
    # df.dropna(subset=['countwaybill'], inplace=True)
    # df.dropna(subset=['realtime_finish_dt'], inplace=True)
    df['md'] = df['real_delv_day'].apply(lambda x: x.strftime('%m-%d'))
    df_t = df.groupby(['md', 'end_node_id'])['waybill_count'].sum().unstack()
    df_t = df_t.fillna(0)
