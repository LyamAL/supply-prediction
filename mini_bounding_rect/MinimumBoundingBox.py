...
def to_grids(w, h, rectangle):
    grids = []
    start_point = (rectangle['rectangle_center'][0] - 0.5 * rectangle['length_parallel'],
                   rectangle['rectangle_center'][1] - 0.5 * rectangle['length_orthogonal'])
    for x_step in list(np.arange(1 / w, 1 + 1 / w, 1 / w)):
        for y_step in list(np.arange(1 / w, 1 + 1 / h, 1 / h)):
            grid = []
            grid.append(start_point)
            grid.append((start_point[0], y_step * rectangle['length_orthogonal'] + start_point[1]))
            grid.append((start_point[0] + x_step * rectangle['length_parallel'],
                         start_point[1] + y_step * rectangle['length_orthogonal']))
            grid.append((start_point[0] + x_step * rectangle['length_parallel'], start_point[1]))
            grid = rotate_points(rectangle['rectangle_center'], rectangle['unit_vector_angle'], grid)
            grids.append(grid)
    return grids
...
def MinimumBoundingBox(grid_width_nums,grid_height_nums, points):
    ...
    return BoundingBox(
        ...
        corner_points=list(rectangle_corners(min_rectangle)),
        grids=list(to_grids(grid_width_nums, grid_height_nums, min_rectangle)),
    )
